...
public class MultiBufferProductPlan implements Plan {
	...
	public String toString(int depth) {
		String rrhs = rhs.toString(depth + 1);
		String llhs = lhs.toString(depth + 1);
		String Output = "";
		for (int i = 0; i < depth; i++) {
			Output += "\t"; 
		}
		Output += "->MultiBufferProductPlan (#blks=" + blocksAccessed() + ", #recs=" + recordsOutput() + ")\n";
		return Output + rrhs + llhs;
	}
}
